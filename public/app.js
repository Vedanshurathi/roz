...
/* % change pill — up is good unless told otherwise (e.g. disputes) */
function trendPill(now, prev, invert) {
  if (!prev) return '';
  const pct = Math.round(((now - prev) / prev) * 100);
  if (pct === 0) return `<span class="trend flat">\u2192 flat vs last week</span>`;
  const up = pct > 0;
  const good = invert ? !up : up;
  return `<span class="trend ${good ? 'up' : 'down'}">${up ? '\u2191' : '\u2193'} ${Math.abs(pct)}% vs last week</span>`;
}

/* Large financial headline card — GMV, AOV etc. Bigger and more
   prominent than the operational kpi() cards below, since these are
   the numbers a founder actually opens this dashboard to see first. */
function finCard(label, value, sub, trend) {
  return `<div class="fin-card">
    <div class="fin-lb">${label}</div>
    <div class="fin-nm">${value}</div>
    <div class="fin-dt">${sub || ''}${trend ? ' &middot; ' + trend : ''}</div>
  </div>`;
}

function payBar(label, amount, total, color) {
  const pct = total > 0 ? Math.round((amount / total) * 100) : 0;
  return `<div class="pay-row">
    <div class="pay-lb"><span>${label}</span><b>${money(amount)}</b></div>
...
/* ---------- views ---------- */
function vHome() {
  const o = D.ov, r = D.readiness, f = D.finance || {gmv_month:0,gmv_week:0,gmv_prev_week:0,aov:0,
    orders_completed_period:0, platform_revenue:0, gmv_all:0, trend:[], cash_total:0, upi_total:0, top_vendors:[]};
  const blocked = r.filter(x => x.state === 'no');
  ...
  document.getElementById('v-home').innerHTML = banner() + `
    ...
    <div class="sec"><h2>Business</h2>
      <span class="r">last 30 days &middot; zero commission, so this is GMV, not platform revenue</span></div>
    <div class="grid fin-grid">
      ${finCard('GMV this month', money(f.gmv_month), 'all completed orders')}
      ${finCard('This week', money(f.gmv_week), null, trendPill(f.gmv_week, f.gmv_prev_week))}
      ${finCard('Average order', money(f.aov), (f.orders_completed_period||0) + ' completed orders')}
      ${finCard('Platform revenue', money(f.platform_revenue), '0% commission model')}
    </div>
    <div class="grid fin-split" style="margin-top:15px;align-items:stretch">
      <div class="card" style="display:flex;flex-direction:column">
        <div style="display:flex;justify-content:space-between;align-items:baseline;margin-bottom:6px">
          <b class="disp" style="font-size:16px">GMV, last 30 days</b>
          <span class="mu" style="font-size:12.5px">${money(f.gmv_all)} all-time</span>
        </div>
        <div style="flex:1">${svgChart(
          f.trend.map(x => ({ v: Number(x.gmv)||0, label: String(x.d).slice(8,10)+'/'+String(x.d).slice(5,7) })),
          { fmt: v => money(v), series: [{ key:'v', name:'GMV', color:'var(--g-lt)',
            fillTop:'rgba(74,222,128,.32)', fillBot:'rgba(74,222,128,0)' }] }
        )}</div>
      </div>
      <div class="card">
        <b class="disp" style="font-size:16px">Cash vs digital</b>
        <div style="margin-top:16px">
          ${payBar('Cash', f.cash_total, f.cash_total + f.upi_total, 'var(--g-lt)')}
          ${payBar('UPI / online', f.upi_total, f.cash_total + f.upi_total, '#60A5FA')}
        </div>
        <div class="sec" style="margin-top:22px;margin-bottom:6px"><h2 style="font-size:14px">Top vendors</h2></div>
        ${f.top_vendors.length ? f.top_vendors.map((v,i) => `
          <div class="tv-row"><span class="tv-rank">${i+1}</span>
            <span class="tv-name">${esc(v.name)}</span>
            <span class="tv-gmv">${money(v.gmv)}</span></div>`).join('')
          : `<div class="mu" style="font-size:13px;padding:8px 0">No completed orders yet this period.</div>`}
      </div>
    </div>
    ...
    <div class="grid k2" style="margin-top:15px">

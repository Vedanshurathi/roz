...
/* Mirrors admin_finance_dashboard()'s real-mode shape exactly, computed
   from the same mock bookings — same reasoning as the real function:
   RozBazaar takes 0% commission, so platform_revenue is honestly 0,
   not a fabricated number. GMV (what actually moved through completed
   orders) is the real financial story here. */
function finance(d, days = 30) {
  const paidStatuses = ['completed', 'paid'];
  const amt = b => Number(b.final_total || b.est_total || 0);
  const today = M.iso(new Date());
  const yesterday = M.iso(M.daysAgo(1));
  const inLast = n => d.bookings.filter(b => paidStatuses.includes(b.status) &&
    new Date(b.booking_date) >= M.daysAgo(n - 1) && new Date(b.booking_date) <= new Date());
  const gmvOn = day => d.bookings.filter(b => paidStatuses.includes(b.status) && b.booking_date === day)
    .reduce((s, b) => s + amt(b), 0);
  const gmvRange = (from, to) => d.bookings.filter(b => paidStatuses.includes(b.status) &&
    b.booking_date >= from && b.booking_date <= to).reduce((s, b) => s + amt(b), 0);

  const periodPaid = inLast(days);
  const aov = periodPaid.length ? Math.round(periodPaid.reduce((s,b)=>s+amt(b),0) / periodPaid.length) : 0;
  /* Mock bookings carry no real payment-method field — rather than
     defaulting every single one to "cash" (which would make the demo
     dashboard show a flat, unrealistic 100/0 split), alternate
     deterministically so the mock view actually demonstrates what the
     real cash/UPI breakdown looks like. */
  const isUpiMock = b => { let h=0; for (const c of String(b.id)) h=(h*31+c.charCodeAt(0))|0; return (h & 1) === 0; };
  const cashTotal = periodPaid.filter(b => !isUpiMock(b)).reduce((s,b)=>s+amt(b),0);
  const upiTotal  = periodPaid.filter(b => isUpiMock(b)).reduce((s,b)=>s+amt(b),0);

  const trend = [];
  for (let i = days - 1; i >= 0; i--) {
    const day = M.iso(M.daysAgo(i));
    trend.push({ d: day, gmv: gmvOn(day),
      orders: d.bookings.filter(b => b.booking_date === day && b.status === 'completed').length });
  }

  const byVendor = {};
  periodPaid.forEach(b => {
    byVendor[b.vendor_id] = byVendor[b.vendor_id] || { gmv: 0, orders: 0 };
    byVendor[b.vendor_id].gmv += amt(b); byVendor[b.vendor_id].orders += 1;
  });
  const top_vendors = Object.entries(byVendor)
    .map(([vid, v]) => ({ name: (d.vendors.find(x => x.id === vid) || {}).name || 'Unknown', ...v }))
    .sort((a,b) => b.gmv - a.gmv).slice(0, 5);

  const weekFrom = M.iso(M.daysAgo(6)), prevWeekFrom = M.iso(M.daysAgo(13)), prevWeekTo = M.iso(M.daysAgo(7));
  return {
    gmv_today: gmvOn(today), gmv_yesterday: gmvOn(yesterday),
    gmv_week: gmvRange(weekFrom, today), gmv_prev_week: gmvRange(prevWeekFrom, prevWeekTo),
    gmv_month: gmvRange(M.iso(M.daysAgo(29)), today),
    gmv_all: d.bookings.filter(b => paidStatuses.includes(b.status)).reduce((s,b)=>s+amt(b),0),
    orders_today: d.bookings.filter(b => b.booking_date === today).length,
    orders_completed_period: periodPaid.length,
    orders_total_period: d.bookings.filter(b => new Date(b.booking_date) >= M.daysAgo(days-1)).length,
    aov, cash_total: cashTotal, upi_total: upiTotal,
    active_vendors: new Set(periodPaid.map(b => b.vendor_id)).size,
    active_customers_period: new Set(periodPaid.map(b => b.customer_id)).size,
    platform_revenue: 0, commission_rate: 0,
    trend, top_vendors, period_days: days
  };
}
...
async function snapshot() {
  if (USE_SUPABASE) {
    const [ov, vendors, areas, day, gaps, perf, bookings, customers, pending, visits, fin] = await Promise.all([
...
async function cancelBooking(id, reason) {
  if (USE_SUPABASE) return rpc('admin_cancel_booking', { p_booking:id, p_reason:reason });
  const b = M.bookings.find(x => x.id === id);
  if (!b) throw new Error('booking_not_found');
  if (['completed','delivered','cancelled'].includes(b.status)) throw new Error('already_' + b.status);
  b.status = 'cancelled'; b.cancel_reason = 'Admin: ' + reason;
...
async function updateProduct(id, price, in_stock, extra = {}) {
  const { name, name_hi, category, unit } = extra;
  if (USE_SUPABASE) return rpc('admin_update_product', {
    p_product:id, p_price: price ?? null, p_in_stock: in_stock ?? null,
    p_name: name ?? null, p_name_hi: name_hi ?? null, p_category: category ?? null, p_unit: unit ?? null
  });
...
async function catalogList() {
  if (USE_SUPABASE) return rpc('admin_catalog_list', {});
  return M.catalogItems || [];
}
async function addCatalogItem({ name_en, name_hi, desc_en, desc_hi, category, unit }) {
  if (USE_SUPABASE) return rpc('admin_add_catalog_item',
    { p_name_en:name_en, p_name_hi:name_hi, p_desc_en:desc_en, p_desc_hi:desc_hi,
      p_category:category, p_default_unit:unit });
  M.catalogItems = M.catalogItems || [];
  const key = 'p' + (M.catalogItems.length + 28);
  const item = { key, name_en, name_hi, desc_en, desc_hi, category, default_unit:unit, is_active:true };
  M.catalogItems.push(item);
  return item;
}
async function removeCatalogItem(key) {
  if (USE_SUPABASE) return rpc('admin_remove_catalog_item', { p_key:key });
  const c = (M.catalogItems || []).find(x => x.key === key);
  if (!c) throw new Error('catalog_item_not_found');
  c.is_active = false;
  return { ...c };
}
...
module.exports = { USE_SUPABASE, snapshot, addVendor, reviewVendor,
                   ...
                   catalogList, addCatalogItem, removeCatalogItem };
